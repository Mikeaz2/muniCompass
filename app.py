...
def effective_state_rate_for(investor_state: str, issuer_state: str, rules: pd.DataFrame) -> float:
    ...
    if tin and tout:   # IN/UT style: tax all muni interest
        return base_rate
    if (not tin) and tout:  # most states: in-state exempt; out-of-state taxed
        return 0.0 if inv == iss else base_rate
    if (not tin) and (not tout):  # no-income-tax states
        return 0.0
    return base_rate  # conservative fallback
...
with st.sidebar:
    state_filter = st.multiselect(
        "Issuer state filter",
        sorted(munis["issuer_state"].unique()),
        key="state_filter"
    )
    type_filter = st.multiselect(
        "Type filter",
        sorted(munis["type"].unique()),
        key="type_filter"
    )
...
- **Indiana & Utah** tax **all** muni interest (even in-state).
- TEY uses **federal + applicable state tax** (combined without double-counting).
- (Prototype) Private-activity bonds may be impacted by **AMT** at the federal level.
""")
...
if only_surplus:
    mask &= df_calc["surplus_flag"].fillna(False)
mask &= (df_calc["RiskScore"] >= min_risk) & (df_calc["TEY_Adj_%"] >= min_adj_tey)
if state_filter:
    mask &= df_calc["issuer_state"].isin(state_filter)
if type_filter:
    mask &= df_calc["type"].isin(type_filter)
